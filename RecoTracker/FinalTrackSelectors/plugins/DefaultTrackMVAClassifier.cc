...
/*
    ...
    if (PROMPT){
      tf::Shape xShape[] = {1,16};
    }
    tf::Tensor* x = new tf::Tensor(2,xShape);

    x->setVector<float>(1,0,gbrVals_);
    tf::Tensor* y = new tf::Tensor();
*/
    x->setVector<float>(1,0,gbrVals_);
    tf::IOs inputs = { session.createIO(x, "ins") };
    tf::IOs outputs = { session.createIO(y, "outs/Sigmoid") };
    session.run(inputs, outputs);

    float output_ = *y->getPtr<float>(0, 0);
